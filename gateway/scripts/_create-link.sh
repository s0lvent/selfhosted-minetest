...
CONTAINER_ID=$(docker run --name $CONTAINER_NAME --network gateway -p 30000 -p 30000/udp -p 18521/udp --cap-add NET_ADMIN --restart unless-stopped -it -e LINK_CLIENT_WG_PUBKEY=$LINK_CLIENT_WG_PUBKEY -d fractalnetworks/gateway-link:latest)
